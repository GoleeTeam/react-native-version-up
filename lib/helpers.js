...
module.exports = {
  ...
  changeVersionInAppJson(pathToFile, version) {
    let packageContent = fs.readFileSync(pathToFile, 'utf8');
    packageContent = packageContent.replace(/("version":\s*")([\d\.]+)(")/g, `$1${version}$3`);
    fs.writeFileSync(pathToFile, packageContent, 'utf8');
  },

  changeAndroidVersionCodeInAppJson(pathToFile, build) {
    let packageContent = fs.readFileSync(pathToFile, 'utf8');
    packageContent = packageContent.replace(/("versionCode":\s*")([\d\.]+)(")/g, `$1${build}$3`);
    fs.writeFileSync(pathToFile, packageContent, 'utf8');
  },

  changeIosBuildNumberInAppJson(pathToFile, build) {
    let packageContent = fs.readFileSync(pathToFile, 'utf8');
    packageContent = packageContent.replace(/("buildNumber":\s*")([\d\.]+)(")/g, `$1${build}$3`);
    fs.writeFileSync(pathToFile, packageContent, 'utf8');
  },
  ...
  changeBuildInGradle(pathToFile, build) {
    let content = fs.readFileSync(pathToFile, 'utf8');
